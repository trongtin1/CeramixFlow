...
`;
     ...
     */
    async chatWithRagAssistant(messages) {
...
`;
        ...
        const missingBasic = [];
        if (!hasQty)
            missingBasic.push('Số lượng cần sản xuất');
        if (!hasHeight)
            missingBasic.push('Chiều cao / Kích thước (cm)');
        if (!hasGlaze)
            missingBasic.push('Loại men gốm mong muốn');
        if (!hasDeadline)
            missingBasic.push('Thời hạn hoàn thành (ngày)');
        if (missingBasic.length > 0) {
            const suggestions = [];
            if (!hasHeight)
                suggestions.push('Chiều cao 35cm');
            if (!hasGlaze)
                suggestions.push('Men lam Bát Tràng');
            if (!hasDeadline)
                suggestions.push('Hoàn thành trong 7 ngày');
            if (!hasQty)
                suggestions.push('Số lượng 100 chiếc');
            return {
        ...
        const hasAdvancedSpecs = lowerFull.includes('co ngót') ||
