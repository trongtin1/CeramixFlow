...
async function main() {
    ...
    const batch1 = await workflow_service_1.workflowService.createBatch({
        batch_code: 'GOM-88',
    ...
    if (batch1) {
        await workflow_service_1.workflowService.advanceStage(batch1.id); // sang PHOI_SUA_MOC
        await workflow_service_1.workflowService.advanceStage(batch1.id); // sang VE_HOA_TIET
        await workflow_service_1.workflowService.advanceStage(batch1.id); // sang TRANG_MEN
        await workflow_service_1.workflowService.advanceStage(batch1.id); // sang VAO_LO_NUNG
    }
    // 2. Mẻ gốm 2: Bộ ấm chén men rạn cổ (Đang ở công đoạn Tạo hình mộc)
    await workflow_service_1.workflowService.createBatch({
        batch_code: 'GOM-102',
