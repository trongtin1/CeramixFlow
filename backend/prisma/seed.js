...
async function seedDatabase() {
    ...
    const sampleBatches = [
        // --- TRẠM 1: TẠO HÌNH MỘC (4 mẻ với đầy đủ 4 cấp độ ưu tiên & tie-break) ---
        {
            batchCode: 'CF-801',
        ...
        // --- TRẠM 5: VÀO LÒ NUNG (Nhiệt độ cao) ---
        {
            batchCode: 'CF-808',
    ...
    for (const item of sampleBatches) {
        ...
        // Xóa nếu trùng mã
        const existing = await prisma_1.default.batch.findUnique({ where: { batchCode: item.batchCode } });
        if (existing) {
            await prisma_1.default.batch.delete({ where: { id: existing.id } });
        }
